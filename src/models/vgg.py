...
def vgg(**kwargs):
    num_classes, arch, dataset, norm_type, groups = map(
        kwargs.get, ['num_classes', 'arch', 'dataset', 'norm_type', 'groups'])
    ...
    if arch == "vgg11":
        ...
        return VGG(make_layers(cfg['VGG13'], norm_type, groups), num_classes, dataset)

    elif arch == "vgg16":
        """VGG 16-layer model (configuration "A") with batch normalization"""
        return VGG(make_layers(cfg['VGG16'], norm_type, groups), num_classes, dataset)

    elif arch == "vgg19":
        """VGG 19-layer model (configuration "A") with batch normalization"""
        return VGG(make_layers(cfg['VGG19'], norm_type, groups), num_classes, dataset)
