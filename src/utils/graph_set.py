...
class Graph(object):
    ...
    def __init__(self, topology, world_size, avg, reg):
    ...
    def get_neighbor_info(self, rank):  # 返回当前rank的所有邻居
        ...
        return neighbor_rank, weight, neighbor, self.dsm
    ...
    def get_comm_group(self):
        ...
        return groups, dist_groups, p2p_groups
    ...
    def sinkhorn_knopp_with_mask(self, matrix, reg=1e-2, max_iter=1000, tol=1e-10, ):
        ...
        for _ in range(max_iter):
            # Regularization to reduce the gap between weights
            matrix += reg * mask
            # Normalize rows with mask
            row_sums = np.sum(matrix, axis=1, keepdims=True)
            matrix = np.divide(matrix, row_sums, where=row_sums!=0)
            # Apply mask to ensure zero elements remain zero
            matrix *= mask
            # Normalize columns with mask
...
#     def make_graph(self):
#         self.neighbor_num = 2
#         self.neighbor = [1 if i == (int(self.rank)+1) % self.size or i ==(int(self.rank)-1 + self.size) % self.size
#                         else 0 for i in range(self.size)]
#         self.edge_weight = [ 1/self.neighbor_num  if i == (int(self.rank)+1) % self.size or i ==(int(self.rank)-1) % self.size
#                             else 0 for i in range(self.size)]
...
#     def make_graph(self):
#         self.neighbor_num = self.size - 1
#         for i in range(self.neighbor_num):
#             self.neighbor.append(1)
#             self.edge_weight.append(1 / self.neighbor_num)
    

# class BiparGraph(Graph):
#     def make_graph(self):
#         self.neighbor_num = self.size / 2
#         for i in range(self.size):
#             if (self.rank + i) % 2 == 0:
#                 self.neighbor.append(1)
#                 self.edge_weight.append(1 / self.neighbor_num)
#             else:
#                 self.neighbor.append(0)
#                 self.edge_weight.append(0.0)
